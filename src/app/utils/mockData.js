...
// Programmatically duplicate the 9 posts to create 27 posts to test the 18-post pagination
export const mockPosts = [
  ...
  ...basePosts.map((p) => ({
    ...
    slug: { current: p.slug.current + '-vol2' },
    publishedAt: new Date(new Date(p.publishedAt).getTime() - 10 * 24 * 60 * 60 * 1000).toISOString(),
    isFeatured: false, // Turn off featured status for duplicated posts
  })),
  ...basePosts.map((p) => ({
    ...p,
    _id: p._id + '-vol3',
    title: p.title + ' (Vol. III)',
    slug: { current: p.slug.current + '-vol3' },
    publishedAt: new Date(new Date(p.publishedAt).getTime() - 20 * 24 * 60 * 60 * 1000).toISOString(),
    isFeatured: false, // Turn off featured status for duplicated posts
  })),
]
